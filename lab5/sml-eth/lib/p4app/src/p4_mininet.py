...
from p4app_util import get_logs_directory
...
class P4Switch(Switch):
    ...
    def __init__(self, name, sw_path = None, json_path = None,
        ...
        logfile = os.path.join(LOGS_DIR, "p4s.{}.log".format(self.name))
...
class P4RuntimeSwitch(P4Switch):
    ...
    def start(self, controllers):
        ...
        if self.start_controller:
            self.sw_conn = p4runtime_lib.bmv2.Bmv2SwitchConnection(
                    ...
                    proto_dump_file=os.path.join(LOGS_DIR, '{}-p4runtime-requests.txt'.format(self.name)))
    ...
    def loadConf(self, sw_conf_or_filename):
        if isinstance(sw_conf_or_filename, dict):
            ...
            conf_path = os.path.join(ROOT_DIR, sw_conf_or_filename)
        ...
        if 'p4info' in sw_conf:
            ...
            self.p4info_path = os.path.join(LOGS_DIR, sw_conf['p4info'])
            self.loadP4Info()
        ...
        if 'bmv2_json' in sw_conf:
            ...
            self.json_path = os.path.join(LOGS_DIR, sw_conf['bmv2_json'])
            self.loadJSON()
